...
@Repository
public interface AccountRepository_Capital_One extends PagingAndSortingRepository<CapitalOneAccounts,Integer> {

    @Query("SELECT c FROM CapitalOneAccounts c WHERE c.customerAccount = :customerAccount")
    public CapitalOneAccounts findByCustomerAccount(@Param("customerAccount") String customerAccount);

    @Query("SELECT c FROM BarclaysAccounts c WHERE c.postalCode = ?1")
    public List<CapitalOneAccounts> findByPostalCode(String postal_code);

    @Query("SELECT c FROM BarclaysAccounts c WHERE c.provinceOrState = ?1")
    public List<CapitalOneAccounts> findByState(String province_state);

    @Query("SELECT c FROM BarclaysAccounts c WHERE c.city = ?1")
    public List<CapitalOneAccounts> findByCity(String city);
