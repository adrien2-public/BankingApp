...
@Component
public class AuthorizationForTransaction {
    ...
    public String doesPersonHaveTheFunds(Exchange exchange){
        ...
        double amount = Double.parseDouble(elementList.get(8));
        ...
        boolean isThisGuyBroke = cardFactory.authorizeTransaction( id, cardNumber,firstName, lastName,expiry, pin, bin, ccv, amount) ;


        if(isThisGuyBroke == true)
        {
            var authorized = custom + "SUCCESS";
            exchange.getIn().setBody(authorized);

        }
        else if (isThisGuyBroke == false)
        {
            var notAuthorized = custom + "INSUFFICIENT FUNDS";
            exchange.getIn().setBody(notAuthorized);
        }
